# Domzop — Portfolio & Property Intelligence

Portfolio manager for **domains** and **real estate**, plus a public **property intelligence** lab: available listings, related projects that may lift or pressure value, and a proprietary Domzop Formula path for past / present / future. A research lab still surfaces domain opportunities and investor reports.
...
docker compose exec -T postgres psql -U domzop -d domzop < supabase/migrations/004_property_intelligence.sql
```
...
- `admin` — dashboard on [http://localhost:3000](http://localhost:3000) (Portfolio, Properties, Candidates, Investors, Reports, Bots)
...
## Portfolio & property intelligence

Two modes of real estate (domains share the same book):

1. **Owned** — holdings with cost basis, current value, and P/L (`/portfolio`)
2. **Available / watching** — public listings the investor is researching (`/properties`)

| Table | Role |
|-------|------|
| `assets` | Name, status (`watchlist` / `owned` / `listed` / `sold` / `discarded`), cost, mark |
| `domain_holdings` | Domain name, registrar, expiry, TLD |
| `real_estate_holdings` | Address, occupancy, rent, taxes, location momentum, condition |
| `valuations` | Mark history |
| `property_catalysts` | Related / side projects that may lift or pressure value |
| `property_intelligence_snapshots` | Domzop Formula score, 1y/3y/5y path, narrative (`domzop-re-v1`) |

**Browse available properties:** Admin → Properties. Cards show estimate, outlook, and intelligence score. Open a card for Past / Present / Future, predicted marks, and related projects.

**Add a watchlist listing:** Properties → Add listing (or Portfolio → Add holding → Real estate, status `watchlist`). Address, type, occupancy, estimate, rent, taxes, location momentum.

**Add an owned property or domain:** Portfolio → Add holding. Buying a research candidate also upserts a domain asset.

The **Domzop Formula** (`domzop-re-v1`) is a proprietary composite (location, yield, risk, catalysts, book quality). It is not an MLS or consumer-portal appraisal. Public UI shows factor names and direction; mix weights stay in code. Narratives use OpenAI when `OPENAI_API_KEY` is set, otherwise a structured heuristic.

Auto-purchase stays off (`AUTO_PURCHASE_ENABLED=false`).
...
domzop/
    admin/                      # Next.js dashboard
      src/app/
        portfolio/              # Owned holdings dashboard
        properties/             # Available listings + intelligence
